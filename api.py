...
platform_name = re.findall('Windows', local_platform)
...
def run(current_url):  # 对每一个cv号进行遍历爬取
    ...
    for url in urls:
        t_dir_name = dir_name
        pool.apply_async(writeTodisk, (t_dir_name, url))
    pool.close()
    pool.join()
    return
...
def findCvNumber(rst1, mid='573495526'):  # 更具mid查找cvhao
    ...
    while (1):
        ...
        rst1.append(re.findall(r'\d+', str(rst)))  # 提取cv号
        rstforprint = re.findall(r'\d+', str(rst))  # 用于打印的cv号列表
